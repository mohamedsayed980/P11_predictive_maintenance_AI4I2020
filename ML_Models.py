"""
Repo_11_Predictive_Maintenance — ML_Models.py  (5 Tabs)
Author : Mohamed · M3
Regression     → Torque [Nm]
Classification → Machine failure  (class_weight='balanced' — 3.4% failure rate)
"""
...
st.set_page_config(page_title="ML Models · Predictive Maintenance · M3",
...
with st.sidebar:
    ...
    st.markdown("Predictive Maintenance · 5 Tabs")
    st.divider()
    st.markdown("### 📂 Dataset")
...
[data-testid="stSidebar"] [data-testid="stBaseButton-secondary"]{background:#c62828 !important;color:#ffffff !important;border:none !important;border-radius:6px !important;}
...
fail_rate  = df[CLF_TARGET].mean() * 100

# ── FEATURE PREP ─────────────────────────────────────────────
FEATURE_COLS = [c for c in [
    "Air temperature [K]","Process temperature [K]",
    "Rotational speed [rpm]","Tool wear [min]",
    "Type_enc","temp_diff","power_proxy",
    "high_wear","high_torque","low_speed","strain_index","overheat",
    "failure_mode_count"
] if c in df.columns]
...
with tabs[0]:
    ...
    with col1:
        ...
        st.markdown(f"**`{REG_TARGET}`** — predict process torque (N·m)")
        st.markdown(f"Mean={y_reg.mean():.1f} · Range={y_reg.min():.1f}–{y_reg.max():.1f} N·m")
    with col2:
        sec("🎯 Classification Target")
        st.markdown(f"**`{CLF_TARGET}`** — predict machine failure (0/1)")
        st.markdown(f"Failure rate={fail_rate:.1f}% → class_weight='balanced'")
    ...
    warn("3.4% failure rate → class_weight='balanced' on all · F1 + Recall + ROC-AUC only · never accuracy")
    cc = st.columns(3)
...
with tabs[1]:
    ...
    info("Predicting: **Torque [N·m]** — process torque during manufacturing")
    ...
    if not S.get("reg_results"):
        ...
        with col2:
            ...
            fig2.update_layout(barmode="group",height=370,title="MAE vs RMSE",xaxis_tickangle=-25)
        ...
        with col3:
            ...
            ax.scatter(yr_test, pred, alpha=0.3, s=8, color=CLR["primary"])
            lims = [min(yr_test.min(),pred.min()), max(yr_test.max(),pred.max())]
            ax.plot(lims,lims,"r--",lw=2,label="Perfect fit")
            ax.set_xlabel("Actual Torque (N·m)"); ax.set_ylabel("Predicted Torque (N·m)")
        ...
        with col4:
            ...
            ax2.set_xlabel("Predicted Torque (N·m)"); ax2.set_ylabel("Residual")
            ax2.set_title("Residual Plot")
            plt.tight_layout(); st.pyplot(fig4); plt.close()

        insight("High R² expected for Torque — it's directly driven by physics (power, speed, load).")
...
with tabs[2]:
    ...
    warn("3.4% failure rate — evaluate ONLY with F1, Recall, ROC-AUC. Accuracy is useless here.")
    ...
    if not S.get("clf_results"):
        ...
        st.dataframe(clf_df.style.background_gradient(subset=["F1","Recall","ROC-AUC"],cmap="RdYlGn")
                                  .format({c:"{:.4f}" for c in ["F1","Recall","Precision","Accuracy","ROC-AUC"]}),
                     use_container_width=True)
        ...
        with col2:
            fig2 = px.bar(clf_df, x="Model", y="Recall",
                          color="Recall", color_continuous_scale=["#c62828","#e65100","#2e7d32"],
                          title="Recall — All Classifiers (catch failures!)",
        ...
        with col3:
            ...
            st.markdown(f"**TP={tp}** (correctly caught failures) · **FN={fn}** (missed failures = dangerous!)")
        ...
        with col4:
            ...
            if hasattr(bm,"predict_proba"):
                ...
                ax2.plot(fpr, tpr, color=CLR["danger"], lw=2.5, label=f"AUC={auc_val:.4f}")
        ...
        warn(f"FN={fn} missed failures — each missed failure costs $8,000+ in corrective maintenance.")
        insight("Recall is MORE important than Precision here — missing a failure is far more costly than a false alarm.")
    ...
    if not S.get("clf_models"):
        ...
        with col1:
            ...
            if hasattr(bm,"feature_importances_"):
                ...
                fig, ax = plt.subplots(figsize=(7,max(5,len(coef)*0.4)))
                ax.barh(coef["Feature"], coef["Coef"], color=CLR["primary"])
                ax.set_xlabel("|Coefficient|"); ax.set_title(f"{best_clf}")
                plt.tight_layout(); st.pyplot(fig); plt.close()
            else:
                info(f"{best_clf} doesn't expose feature importances directly.")

        with col2:
            sec("📈 Regression Importance — Torque Prediction")
            if not reg_df.empty:
                best_reg = reg_df.iloc[0]["Model"]
                ...
                if hasattr(rm,"feature_importances_"):
                    imp2 = pd.DataFrame({"Feature":feats,"Importance":rm.feature_importances_})\
                             .sort_values("Importance",ascending=True)
                    fig2, ax2 = plt.subplots(figsize=(7,max(5,len(imp2)*0.4)))
                    ax2.barh(imp2["Feature"], imp2["Importance"], color=CLR["teal"])
                    ax2.set_xlabel("Importance"); ax2.set_title(f"{best_reg} — Torque Predictors")
                    plt.tight_layout(); st.pyplot(fig2); plt.close()
                elif hasattr(rm,"coef_"):
                    coef2 = pd.DataFrame({"Feature":feats,"Coef":np.abs(rm.coef_)})\
                              .sort_values("Coef",ascending=True)
                    fig2, ax2 = plt.subplots(figsize=(7,max(5,len(coef2)*0.4)))
        ...
        insight("strain_index, high_wear, and Torque typically rank as top failure predictors.")
        insight("tool_wear and power_proxy rank highest for Torque regression — physics confirmed by ML.")

# ══════════════════════════════════════════════════════════════
# TAB 5 — PREDICT — FAILURE RISK SCORER
# ══════════════════════════════════════════════════════════════
with tabs[4]:
    sec("🔮 Tab 5 — Interactive Failure Risk Scorer")

    if not S.get("clf_models"):
        warn("Train at least one model in Tab 1.")
    else:
        info("Enter current machine readings to predict failure probability in real-time.")

        col1, col2, col3 = st.columns(3)
        with col1:
            sec("🌡 Temperature Readings")
            ...
            product_type = st.selectbox("Product Type", ["L (Low)","M (Medium)","H (High)"])

        with col2:
            sec("⚙️ Process Parameters")
            rpm       = st.slider("Rotational Speed (rpm)", 1100, 2900, 1500, 10)
            torque    = st.slider("Torque (N·m)",            3.0,  80.0, 40.0, 0.5)
            tool_wear = st.slider("Tool Wear (min)",          0,   260,   100,   5)

        with col3:
            sec("📊 Derived Indicators")
            temp_diff_val   = proc_temp - air_temp
            power_val       = torque * rpm / 9550
            type_enc_val    = {"L (Low)":0,"M (Medium)":1,"H (High)":2}[product_type]
            high_wear_val   = 1 if tool_wear > 200 else 0
            high_torque_val = 1 if torque > 46.8 else 0
            low_speed_val   = 1 if rpm < 1423 else 0
            strain_val      = 1 if (high_torque_val == 1 and low_speed_val == 1) else 0
            q90_temp        = df["Process temperature [K]"].quantile(0.90)
            overheat_val    = 1 if proc_temp > q90_temp else 0

            st.metric("Temp Differential", f"{temp_diff_val:.2f} K",
                      delta=f"{'⚠️ Low' if temp_diff_val < 8.5 else '✅ OK'}")
            st.metric("Power Proxy",       f"{power_val:.3f} kW",
                      delta=f"{'⚠️ High' if power_val > 8.5 else '✅ OK'}")
            st.metric("High Wear Flag",    "⚠️ YES" if high_wear_val else "✅ NO")
            st.metric("Strain Index",      "🚨 HIGH" if strain_val else "✅ Normal")
        ...
        if st.button("🔮 Predict Failure Risk", type="primary", use_container_width=True):
            input_row = pd.DataFrame([{
                ...
                "Type_enc":                  type_enc_val,
                "temp_diff":                 temp_diff_val,
                "power_proxy":               power_val,
                "high_wear":                 high_wear_val,
                "high_torque":               high_torque_val,
                "low_speed":                 low_speed_val,
                "strain_index":              strain_val,
                "overheat":                  overheat_val,
                "failure_mode_count":        0,
            }])
            # Align columns
            input_aligned = pd.DataFrame([{k: input_row.get(k, pd.Series([0]))[0]
                                           for k in S["X_cols"]}])
            input_sc = S["scaler"].transform(input_aligned)

            sec("🎯 Failure Risk — All Classifiers")
            pred_rows = []
            for name, model in S["clf_models"].items():
                ...
                pred_rows.append({
                    "Model":       name,
                    "Prediction":  "🚨 FAILURE RISK" if pred==1 else "✅ NORMAL",
                    "Probability": f"{prob*100:.1f}%" if prob is not None else "N/A",
                })
            pred_df = pd.DataFrame(pred_rows)
            st.dataframe(pred_df, use_container_width=True)

            fail_votes = sum(1 for r in pred_rows if "FAILURE" in r["Prediction"])
            if fail_votes > len(pred_rows)/2:
                st.error(f"🚨 FAILURE RISK DETECTED — {fail_votes}/{len(pred_rows)} models predict failure. Schedule maintenance NOW.")
            else:
                st.success(f"✅ Normal operation — only {fail_votes}/{len(pred_rows)} models flag risk.")

            st.markdown("---")
            sec("📋 Risk Factor Analysis")
            risks = []
            if high_wear_val:
                risks.append(("🔴 CRITICAL","Tool wear > 200 min — replace immediately"))
            if strain_val:
                risks.append(("🔴 HIGH","Mechanical strain: high torque + low speed simultaneously"))
            if overheat_val:
                risks.append(("🟡 MEDIUM","Process temperature in top 10% — cooling check needed"))
            if power_val > 8.5:
                risks.append(("🟡 MEDIUM",f"Power proxy = {power_val:.2f} kW — machine overloading"))
            if temp_diff_val < 8.5:
                risks.append(("🟡 MEDIUM",f"Temp diff = {temp_diff_val:.2f}K — heat dissipation degrading"))
            if tool_wear > 150:
                risks.append(("🟡 MEDIUM",f"Tool wear at {tool_wear} min — approaching critical threshold"))

            if risks:
                for level, msg in risks:
                    color = "warn" if "🔴" in level else "info"
                    st.markdown(f'<div class="{color}-box"><p>{level}: {msg}</p></div>',
                                unsafe_allow_html=True)
            else:
                st.markdown('<div class="insight-box"><p>✅ No risk factors detected — machine operating within normal parameters.</p></div>',
                            unsafe_allow_html=True)
